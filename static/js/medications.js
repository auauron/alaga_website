document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderMedicationHistory() {
      ...
      if (medicationHistory.length > 0) {
        ...
        medicationHistory.forEach((item) => {
          ...
          historyItem.className = "p-4 flex items-center justify-between"
          ...
          historyItem.innerHTML = `
            ...
            <div class="text-right">
              <p class="font-medium">${formatDate(new Date(item.date))}</p>
    ...
    function createMedicationElement(medication, section, date) {
      ...
      // Status display for today's medications
      let statusDisplay = ""
  
      // For today's medications, check if there's a history entry
      if (section === "today") {
        const today = new Date().toISOString().split("T")[0]
        const historyEntry = medicationHistory.find((item) => item.medicationId == medication.id && item.date === today)
  
        if (historyEntry) {
          const statusClass = historyEntry.status === "taken" ? "text-green-500" : "text-red-500"
          statusDisplay = `<span class="text-sm font-medium ${statusClass} ml-2">${historyEntry.status === "taken" ? "Taken" : "Skipped"}</span>`
        }
      }
      ...
      if (section === "today") {
        div.innerHTML = `
          ...
          <div class="flex items-center">
            <span class="mr-4">${time} ${statusDisplay}</span>
            <button class="h-8 w-8 rounded-full bg-gray-100 text-gray-400 flex items-center justify-center mr-2 skipped-btn" title="Mark as skipped">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
            <button class="h-8 w-8 rounded-full bg-gray-100 text-gray-400 flex items-center justify-center taken-btn" title="Mark as taken">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    function updateMedicationStatus(id, status) {
      ...
      fetch("/api/medication-history", {
        ...
        .then((data) => {
          ...
          loadMedicationHistory()
          // Re-render medications to show updated status
          renderMedications()
        })
        .catch((error) => {
